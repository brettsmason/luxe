...
add_action( 'after_switch_theme', 'luxe_switch_theme' );
add_action( 'load-customize.php', 'luxe_load_customize' );
add_action( 'template_redirect', 'luxe_preview' );
 ...
 * @return void
 */
function luxe_switch_theme( $old_name ) {

	switch_theme( $old_name ? $old_name : WP_DEFAULT_THEME );

	unset( $_GET['activated'] );

	add_action( 'admin_notices', 'luxe_upgrade_notice' );
}
 ...
 * @return void
 */
function luxe_upgrade_notice() {
	printf( '<div class="error"><p>%s</p></div>', esc_html( luxe_compat_message() ) );
}
 ...
 * @return void
 */
function luxe_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( esc_html( luxe_compat_message() ) );
	}
}
